...
void limits_init()
{
  ...
  attachInterrupt(digitalPinToInterrupt(X_LIMIT_PIN), state_Limit_pins, CHANGE);
  attachInterrupt(digitalPinToInterrupt(Y_LIMIT_PIN), state_Limit_pins, CHANGE);
  attachInterrupt(digitalPinToInterrupt(Z_LIMIT_PIN), state_Limit_pins, CHANGE);
}

void state_Limit_pins(void) // DEFAULT: Limit pin change interrupt process.
{
...
#else
  detachInterrupt(digitalPinToInterrupt(X_LIMIT_PIN));
  detachInterrupt(digitalPinToInterrupt(Y_LIMIT_PIN));
  detachInterrupt(digitalPinToInterrupt(Z_LIMIT_PIN));
#endif
}
...
void limits_go_home(uint8_t cycle_mask)
{
  ...
  for (idx = 0; idx < N_AXIS; idx++)
  {
    ...
    {
#ifdef PLT_V2 || HOMING_FORCE_SET_ORIGIN
      set_axis_position = 0;
#else
      if (bit_istrue(settings.homing_dir_mask, bit(idx)))
      {
