...
static void report_util_encoder_values(float *axis_value) {
  uint8_t idx;
  for (idx=0; idx<N_AXIS; idx++) {
    printFloat_CoordValue(axis_value[idx]);
    if (idx < (N_AXIS-1)) { serial_write(','); }
...
void report_realtime_status()
{
  ...
  float print_real_position[N_AXIS];
  //memcpy(current_real_position,sys_real_position,sizeof(sys_real_position));
 
  system_convert_array_encoder_steps_to_mpos(print_real_position,sys_real_position);
  #endif
  ...
  report_util_axis_values(print_position);
   #ifdef PLT_V2_ENCODER 
    printPgmString(PSTR("|RealPos:"));
    report_util_encoder_values(print_real_position);
