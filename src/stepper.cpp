...
// Then pulse the stepping pins
  ...
  if ((st.dir_outbits & (1 << X_DIRECTION_BIT)) == 0)
    ...
    MOTORX_LEFT;
  if ((st.dir_outbits & (1 << Y_DIRECTION_BIT)) == 0)
    MOTORY_RIGHT;
  else
    MOTORY_LEFT;
  if ((st.dir_outbits & (1 << Z_DIRECTION_BIT)) == 0)
    ...
    MOTORZ_LEFT;
  if ((LastPort & 0xE0) != (st.dir_outbits & 0xE0)) //	If there is a change of direction !!!
    delay_us(5);
  LastPort = st.dir_outbits;
...
void st_generate_step_dir_invert_masks()
{
  ...
  for (idx = 0; idx < N_AXIS; idx++)
  {
    ...
    {
      step_port_invert_mask[idx] = get_step_pin_mask(idx);
    }
    else
    {
      step_port_invert_mask[idx] = 0;
    }

    if (bit_istrue(settings.dir_invert_mask, bit(idx)))
    {
      dir_port_invert_mask[idx] = get_direction_pin_mask(idx);
    }
    else
    {
      dir_port_invert_mask[idx] = 0;
    }
  }
#else
